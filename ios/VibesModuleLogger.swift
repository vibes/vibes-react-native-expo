...
class VibesModuleLogger: NSObject {
  ...
  fileprivate func sendLog(
    ...
    // get env from plist config file if set, else use default UAT
    let env = Configuration.configValue(.vibesAppEnv) ?? "UAT"
  ...
  fileprivate func dispatch(_ body: [AnyHashable: Any], requestType: String) {
    do {
      ...
      if let log = NSString(
        data: pretty, encoding: String.Encoding.utf8.rawValue)
      {
        VibesModuleLogger.data.insert(
          NSString(string: "\(timestamp) \(log)"), at: 0)
      }
      sendLog(
        uuid: uuid, timestamp: timestamp, body: body, request_type: requestType)
    } catch {
...
extension VibesModuleLogger: VibesLogger {
  func log(_ logObject: LogObject) {
    let body = [
      "type":
        "\(logObject.level.logPrefix.lowercased().capitalizingFirstLetter())",
    ...
    if let url = response.url {
      ...
      let body =
        [
          ...
          kBodyHeaders: (response as? HTTPURLResponse)?.headersDescription
            as Any,
